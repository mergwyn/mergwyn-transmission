...
class transmission::service {
  ...
  exec { 'replace_transmission_config':
    command     => "${::transmission::params::stop_cmd} && cp -a ${::transmission::params::config_dir}/settings.json.puppet ${::transmission::params::config_dir}/settings.json && ${::transmission::params::start_cmd}",
    refreshonly => true,
  }
